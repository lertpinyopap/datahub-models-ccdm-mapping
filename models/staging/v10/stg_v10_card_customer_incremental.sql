{{ config(materialized='view', schema='INTERMEDIATE', alias='STG_V10_CARD_CUSTOMER_INCREMENTAL', tags=['v10_mapping_staging']) }}
...
ranked_source as (
    select
        ...
        row_number() over (
            ...
            order by
                UPDATED_DATETIME desc nulls last,
                CREATED_DATETIME asc nulls last,
                CUSTOMER_ORG,
                ACCOUNT_ORG,
                ACCOUNT_ID
        ) as SOURCE_ROW_NUMBER,
        min(CREATED_DATETIME) over (
            partition by CUSTOMER_ID
        ) as OLDEST_CREATED_DATETIME
    from joined_source
)
...
select
    ...
    SURNAME,
    DATE_OF_BIRTH,
    OLDEST_CREATED_DATETIME as CREATED_DATETIME,
    UPDATED_DATETIME,
    AMBS_INT_STATUS
from ranked_source
