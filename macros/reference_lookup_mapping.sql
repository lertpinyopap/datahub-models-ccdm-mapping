    ...
    left join (
        ...
        where coalesce(trim(mapping.IS_DELETED_FLAG), 'N') <> 'Y'
          ...
          and (
                ...
                or cast(mapping.VALID_FROM_DATETIME as timestamp_ntz) <= {{ resolved_as_of_date_expr }}
          )
          and (
                mapping.VALID_TO_DATETIME is null
                or cast(mapping.VALID_TO_DATETIME as timestamp_ntz) >= {{ resolved_as_of_date_expr }}
          )
          and (
                code.VALID_FROM_DATETIME is null
                or cast(code.VALID_FROM_DATETIME as timestamp_ntz) <= {{ resolved_as_of_date_expr }}
          )
          and (
                code.VALID_TO_DATETIME is null
                or cast(code.VALID_TO_DATETIME as timestamp_ntz) >= {{ resolved_as_of_date_expr }}
          )
